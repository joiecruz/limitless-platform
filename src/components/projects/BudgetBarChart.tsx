...
export default function BudgetBarChart({ categories }: BudgetBarChartProps) {
  ...
  const data = [
    ...top.map((cat) => ({
      ...
      amount: parseAmount(cat.amount), // Ensure amount is a number for LabelList
    })),
    ...
    <ResponsiveContainer width="100%" height="100%">
      <BarChart data={data} layout="vertical" margin={{ left: 20, right: 20, top: 20, bottom: 20 }}>
        ...
        <Tooltip formatter={(value, _name, props) => {
          ...
          return [`$${Number(value).toLocaleString()} (${(entry.percentage || 0).toFixed(1)}%)`, entry.name || ''];
        }} />
